...
# > python3.8 rubikDetectorAR.py
# > python3.8 rubikDetectorAR.py --dev=help
...
from umucv.htrans   import desp, scale, Pose, jr, jc, col, row, rotation
...
# Possible key moves
KEY_MOVES = [ 'x', 'y', 'z' ]
KEY_MOVES = [ ord(k) for k in KEY_MOVES ]

### AR CONSTANTS
# Distance from camera to origin (to avoid zero value)
DC = 1
# Camera FOV
MY_FOV = 52
# L shape marker
marker = np.array(
       [[0,   0,   0],
        ...
        [1,   0.5, 0],
        [1,   0,   0]])
# Size of the sticker and cube in AR
STK_SIZE_AR = 30
CUBE_S_AR = STK_SIZE_AR*3
# AR textures objects
objectsAR = []
# Turning steps
TURN_STEPS = 25
...
def rotateList90(ilist, ddir):
    '''
    @brief Rotates a 9 list elements as a 3x3 color matrix.
    I'm sure there is simpler way, but I'm so tired.
    @return: The result list
    '''
    rlist = []
    if ddir == 1:   # Clockwise
        rlist.append(ilist[6])
        rlist.append(ilist[3])
        rlist.append(ilist[0])
        rlist.append(ilist[7])
        rlist.append(ilist[4])
        rlist.append(ilist[1])
        rlist.append(ilist[8])
        rlist.append(ilist[5])
        rlist.append(ilist[2])
    else:  # Anticlockwise
        rlist.append(ilist[2])
        rlist.append(ilist[5])
        rlist.append(ilist[8])
...
def rotateList180(ilist):
    '''
    @brief Rotates a 9 list elements as a 3x3 color matrix.
    I'm sure there is simpler way, but I'm so tired.
    @return: The result list
    '''
    return rotateList90(rotateList90(ilist, 1), 1)
...
def polygons(cs, n, prec=2):
    """
    @brief Find polygons in the image.
    @return: The plygons in the image
    """
...
def rots(c):
    """
    @brief Calculate all possible rotations of the polygon.
    @return: All possible rotations of the polygon
    """
    return [np.roll(c,k,0) for k in range(len(c))]

def bestPose(K, view, model):
    """
    @brief Select the best pose of the marker.
    @return: The poses sorted by lower error
    """
    poses = [ Pose(K, v.astype(float), model) for v in rots(view) ]
    return sorted(poses,key=lambda p: p.rms)[0]

def findPoses(img, poly):
    """
    @brief Pose estimation function.
    @return: The poses which fit with the marker
    """
...
### CAMERA POSITION AND AR FUNCTIONS
def setTransform(H, obj):
    """
    @brief Resets the transform of an object.
    Adapts numpy's array type to the one used by pyqtgraph.
    """
    obj.setTransform(QtGui.QMatrix4x4(*(H.flatten())))

def img2tex(img):
    """
    @brief Convert images into "textures" to display them in the 3D scene.
    @return: The texture objecto of the image
    """
...
def img2texA(img):
    """
    @brief Convert images with alpha channel into "textures" to display them in the 3D scene.
    @return: The texture objecto of the image
    """
...
def Kfov(sz,hfovd):
    """
    @brief Simple calibration matrix from horizontal fov in degrees.
...
def setCameraPosition(w, pos=None, distance=None, elevation=None, azimuth=None, fov=None):
    """
    @brief Sets the camera position in the 3D space.
    
    Pyqtgraph has a similar but more limited function and some options do not work.
...
def prepareAR(w, size, fov, dist=20):
    """
    @brief Positionate the opengl "camera" and display the camera image at the correct distance.
    """
    WIDTH, HEIGHT = size 
    # Set camera in the correct position
    setCameraPosition(w, distance=DC, elevation=-90, azimuth=-90, fov=fov)
    
    # Prepare the texture object that will contain the live camera image
    W2 = WIDTH/2
    d = dist
    s = (d + DC)/W2 * np.tan(np.radians(fov)/2)
    camera_image = gl.GLImageItem(data=np.zeros([100,100,4]))
    setTransform( scale((s,s,1)) @ desp((-WIDTH//2,-HEIGHT//2, d)) , camera_image)
...
def cameraTransf(p):
    """
    @brief Build the scene transformation matrix from a Pose object.
...
def mkTexture(w, img, T):
    """
    @brief Creates an image object and returns a function that updates its position with the scene transformation deduced from the camera matrix.

    @param: T una transformación T que lleva la posición inicial de la imagen, in the plane z=0, and with width = 1, to the desired position

    @return: A function that updates its position with the scene transformation deduced from the camera matrix
    """
    W = img.shape[1]
    if img.shape[2] == 3:
    ...
    D = desp((0,0,-DC))
    Dg = desp((-0.5,-0.5,-0.5))
    Dgi = desp((0.5,0.5,0.5))
    def update(H, G):
        setTransform( D @ H @ Dgi @ G @ Dg @ T @ S , obj )
    return update

def rx(ad):
    """
    @brief Create an homogeneous rotation (4x4) on the x axis.
    @return: The rotation transform on the x axis
    """
    return rotation((1,0,0),np.radians(ad),homog=True)

def ry(ad):
    """
    @brief Create an homogeneous rotation (4x4) on the y axis.
    @return: The rotation transform on the y axis
    """
    return rotation((0,1,0),np.radians(ad),homog=True)

def rz(ad):
    """
    @brief Create an homogeneous rotation (4x4) on the z axis.
    @return: The rotation transform on the z axis
    """
    return rotation((0,0,1),np.radians(ad),homog=True)

def rd(d, ad):
    """
    @brief Create an homogeneous rotation (4x4) on the d axis.
    @return: The rotation transform on the d axis
    """
    return rotation(d,np.radians(ad),homog=True)

def updateObjectsAR(cube):
    """
    @brief Update the cube textures object (objectsAR).
    """
    ## First remove the old textures frow the widget (except the camera input)
    for obj in wi.items[1:]:
        wi.removeItem(obj)
    ...
    for face in cube:
        face_AR = []
        # For every sticker, create the image
        for color in face:
            x = np.zeros([STK_SIZE_AR, STK_SIZE_AR, 3], np.uint8)
            x[:,:] = color
            face_AR.append(x)
        # Create the face image combining the nine color grids
        f1 = np.hstack([face_AR[0], face_AR[1], face_AR[2]])
        f2 = np.hstack([face_AR[3], face_AR[4], face_AR[5]])
        f3 = np.hstack([face_AR[6], face_AR[7], face_AR[8]])
        face_AR = np.vstack([f1, f2, f3])
        # Add to the AR cube
        cubeAR.append(face_AR)
    # For the rest of the cube, create transparent faces
    for i in range(len(cube), 6):
        # Create and emtpy sticker
        emptyStck = np.zeros([STK_SIZE_AR, STK_SIZE_AR, 4], np.uint8)
        # Create the face image combining the nine color grids
        f1 = np.hstack([emptyStck, emptyStck, emptyStck])
        f2 = np.hstack([emptyStck, emptyStck, emptyStck])
        f3 = np.hstack([emptyStck, emptyStck, emptyStck])
        face_AR = np.vstack([f1, f2, f3])
        # Add to the AR cube
        cubeAR.append(face_AR)
    ...
    for face in cubeAR:
        for q in stickers:
            cv.rectangle(face, (q[0], q[1]), (q[0]+STK_SIZE_AR, q[1]+STK_SIZE_AR),
                        color=(0, 0, 0, 255), thickness=2)
        # Draw the outer border (it's wider)
        cv.rectangle(face, (0, 0), (CUBE_S_AR, CUBE_S_AR),
                    color=(0, 0, 0, 255), thickness=4)
        
    ## Perfrom the 3D transformations using the respective rotation, translation and scaling matrices
    T0 = desp((0,1,0)) @ ry(180) @ rz(180)
    global objectsAR
    objectsAR = [ mkTexture(wi, cubeAR[0], desp((0,0,1)) @ T0)                  # U
            , mkTexture(wi, cubeAR[1], rx(90) @ T0)                             # F
            , mkTexture(wi, cubeAR[2], desp((1,0,0)) @ rz(90) @ rx(90) @ T0)    # R
            , mkTexture(wi, cubeAR[3], desp((1,1,0)) @ rz(180) @ rx(90) @ T0)   # B
            , mkTexture(wi, cubeAR[4], desp((0,1,0)) @ rz(-90) @ rx(90) @ T0)   # L
            , mkTexture(wi, cubeAR[5], desp((0,1,0)) @ rz(-90))                  # D
            ]

def createRotationMatrix(key, ddir, angle, update=False):
    '''
    @brief Creates the rotation matrix based on key pressed.

    If update is True, updates the cube structure.
    ...
    if key == ord('x'):
        ...
        axis = (0, 0, 1)
    # Rotation matrix
    G = rd(axis, ddir*angle)
    if not update:
        return G
    # If update is required
    newCube = []
    global cube
    if key == ord('x') and ddir == 1:
        newCube.append(rotateList180(cube[3]))
        newCube.append(cube[0])
        newCube.append(rotateList90(cube[2], -1))
        newCube.append(rotateList90(cube[5], 1))
        newCube.append(rotateList90(cube[4], 1))
        newCube.append(rotateList90(cube[1], 1))
    elif key == ord('x') and ddir == -1:
        newCube.append(cube[1])
        newCube.append(rotateList90(cube[5], -1))
        newCube.append(rotateList90(cube[2], 1))
        newCube.append(rotateList180(cube[0]))
        newCube.append(rotateList90(cube[4], -1))
        newCube.append(rotateList90(cube[3], -1))
    elif key == ord('y') and ddir == 1:
        newCube.append(rotateList90(cube[4], 1))
        newCube.append(rotateList90(cube[1], 1))
        newCube.append(rotateList90(cube[0], 1))
        newCube.append(rotateList90(cube[3], -1))
        newCube.append(cube[5])
        newCube.append(rotateList180(cube[2]))
    elif key == ord('y') and ddir == -1:
        newCube.append(rotateList90(cube[2], -1))
        newCube.append(rotateList90(cube[1], -1))
        newCube.append(rotateList180(cube[5]))
        newCube.append(rotateList90(cube[3], 1))
        newCube.append(rotateList90(cube[0], -1))
        newCube.append(cube[4])
    elif key == ord('z') and ddir == 1:
        newCube.append(rotateList90(cube[0], -1))
        newCube.append(cube[4])
        newCube.append(cube[1])
        newCube.append(cube[2])
        newCube.append(cube[3])
        newCube.append(rotateList90(cube[5], 1))
    elif key == ord('z') and ddir == -1:
        newCube.append(rotateList90(cube[0], 1))
        newCube.append(cube[2])
...
x, y, z: rotate the cube
(only when full scanned)
...
## AR initialization ##
# To construct the approximate calibration matrix
K = Kfov(size, MY_FOV)
# Visualization of live camera
update_view = prepareAR(wi, size, MY_FOV)

## CREATE THE TEXTURES ##
updateObjectsAR([])

###     MAIN LOOP    ###
# Pyqtgraph callback to serve as a capture loop
def update():
    # Global variables
    global rem_delay, rem_turning, turn_key, turn_dir, cubeUpdate, \
    KEY_MOVES, SHOW_BOXES, SHOW_MODEL, SHOW_CONTOURS, SHOW_CANNY, \
    SHOW_GRID, PAUSE, cube, centers, moves
    # Key and frame
    key, frame = next(stream)

    # We update the camera image in the scene
    update_view(frame)

    # We estimate the pose
    poses = findPoses(frame, marker)
    # Take the first pose
    if poses != []:
        # We extract the scene transformation from the camera matrix
        H = cameraTransf(poses[0])

        # If cube is turning
        nextUpdate = False
        if rem_turning > 0:
            angle = (1-rem_turning/TURN_STEPS)*90
            rem_turning -= 1
            if rem_turning == 0:
                G = createRotationMatrix(turn_key, turn_dir, angle, True)
                nextUpdate = True
            else:
                G = createRotationMatrix(turn_key, turn_dir, angle)
        else: 
            G = np.eye(4)
        ...
        if cubeUpdate:
            updateObjectsAR(cube)
            cubeUpdate = False
         # We move the virtual objects to the right position 
        # So that they can be seen with the correct perspective.
        for obj in objectsAR: 
            obj(H, G)
        # Show the objects
        for obj in wi.items[1:]:
            obj.setVisible(True)

        # If objects must be updated in the next iteration
        if nextUpdate:
            cubeUpdate = True
        
    # Sets cube to turn
    if rem_turning == 0 and key in KEY_MOVES and len(cube) == 6:
        turn_key = key
        rem_turning = TURN_STEPS

    # If invert key is pressed
    if rem_turning == 0 and key == ord('i'):
        turn_dir *= -1

    # FRAME DELAY AND PAUSE
    if key == ord('p'):      # Pause/resume scanning
            PAUSE = not PAUSE
    if rem_delay > 0 or PAUSE:   # If there is remaining delay moves on to the next iteration
        if rem_delay > 0: rem_delay -= 1
